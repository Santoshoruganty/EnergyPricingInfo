...
# 1. Fetch data for all ISOs in parallel
iso_loaders = {
    "ercot": get_ercot_data,
    "isone": get_isone_data,
    "miso": get_miso_data,
    "pjm": get_pjm_data,
}
...
with ThreadPoolExecutor(max_workers=4) as executor:
    futures = {name: executor.submit(func) for name, func in iso_loaders.items()}
...
# 2. Run calculations in parallel for the ones that succeeded
summaries_by_iso = {}

def run_summary(name, result):
    return name, calculations(result['previous_df'], result['current_df'], result['f_date'], result['l_date'])

with ThreadPoolExecutor(max_workers=4) as executor:
    summary_futures = {
        name: executor.submit(run_summary, name, result)
        for name, result in results.items() if result is not None
    }
    for name, future in summary_futures.items():
        try:
            iso_name, summary = future.result()
            summaries_by_iso[iso_name] = summary
        except Exception as e:
            summaries_by_iso[name] = None
            print(f"❌ Failed to calculate summary for {name.upper()}: {e}")
...
for tab_name, tab in zip(["ercot", "nyiso", "isone", "miso", "pjm"], tabs):
    with tab:
        ...
        if tab_name in results and results[tab_name]:
            result = results[tab_name]
            summaries = summaries_by_iso.get(tab_name)
            if summaries:
                st.header("📰 Newsletter Summary")
                display_summaries(summaries)
                render_forecast_visuals(result['previous_df'], result['current_df'], iso_name=tab_name)
            else:
                st.warning(f"Summary generation failed for {tab_name.upper()}.")
        else:
            st.warning(f"No data available for {tab_name.upper()} or failed to load.")
