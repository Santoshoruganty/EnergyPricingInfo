...
def fetch_miso_data(operating_date):
    ...
    try:
        ...
        print(f"Error fetching MISO data: {e}")
        return None
...
def get_miso_data():
    ...
    # Initial fetch
    current_json = fetch_miso_data(current_op)
    previous_json = fetch_miso_data(previous_op)
    ...
    current_df = pivot_miso_json(current_json) if current_json else None
    previous_df = pivot_miso_json(previous_json) if previous_json else None
