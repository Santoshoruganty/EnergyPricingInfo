...
def load_latest_cached_summary(iso, cache):
    summaries = [(k, v) for k, v in cache.items() if k.startswith(iso)]
    if not summaries:
        return None
    return sorted(summaries, key=lambda x: x[0].split("_")[1], reverse=True)[0][1]

#########################################
groq_api_key = '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'
...
# tabs = st.tabs(["ISONE", "MISO", "PJM"])
for tab_name, tab in zip(["ercot", "nyiso", "isone", "miso", "pjm"], tabs):
# for tab_name, tab in zip(["isone", "miso", "pjm"], tabs):
        ...
        if tab_name == 'ercot':
            with st.spinner("Fetching ERCOT data..."):
                try:
                    ...
                    summary_key = f"{tab_name}_{date_key}"
                    cache = load_summary_cache()

                    if not result.get("success"):
                        st.warning("⚠️ ERCOT data not available. Showing last summary.")
                        fallback = load_latest_cached_summary(tab_name, cache)
                        if fallback:
                            display_summaries(fallback)
                        else:
                            st.error("❌ No cached summary found.")
                        continue

                    if summary_key in cache:
                        st.success("✅ Using cached summary.")
                        summaries = cache[summary_key]
                    else:
                        st.warning("📊 New data found. Generating summary...")
                        summaries = calculations(result['previous_df'], result['current_df'], result['f_date'], result['l_date'])
                        cache[summary_key] = summaries
                    ...
                    st.error(f"Failed to fetch {tab_name.upper()} data: {e}")
            ...
            with st.spinner("Fetching ISONE data..."):
                try:
                    ...
                    if not result.get("success"):
                        st.warning("⚠️ ISONE data not available. Showing latest cached summary.")
                        fallback = load_latest_cached_summary(tab_name, cache)
                        if fallback:
                            display_summaries(fallback)
                        else:
            ...
            with st.spinner("Fetching MISO data..."):
                try:
                    ...
                    if not result.get("success"):
                        st.warning("⚠️ MISO data not available. Showing latest cached summary.")
                        fallback = load_latest_cached_summary(tab_name, cache)
                        if fallback:
                            display_summaries(fallback)
                        else:
            ...
            with st.spinner("Fetching PJM data..."):
                try:
                    ...
                    if not result.get("success"):
                        st.warning("⚠️ PJM data not available. Showing latest cached summary.")
                        fallback = load_latest_cached_summary(tab_name, cache)
                        if fallback:
                            display_summaries(fallback)
                        else:
            ...
            with st.spinner("Fetching NYISO data..."):
                try:
                    result = get_nyiso_data()
                    date_key = result.get("current_op_date", "unknown")
                    summary_key = f"{tab_name}_{date_key}"
                    cache = load_summary_cache()

                    if not result.get("success"):
                        st.warning("⚠️ NYISO data not available. Showing latest cached summary.")
                        fallback = load_latest_cached_summary(tab_name, cache)
                        if fallback:
                            display_summaries(fallback)
                        else:
