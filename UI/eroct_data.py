...
def fetch_data(operating_date):
    ...
    # Forecast window: next month to 2030
    next_month = operating_date.month + 1
    year = operating_date.year
    if next_month > 12:
        next_month = 1
        year += 1
    ...
    try:
        response = requests.get(url, params=query, headers=headers, verify=False)
        print("🔍 Fetched for:", op_date_str)
        print("URL:", response.url)
        print("Status:", response.status_code)
        print("Preview:", response.text[:200])

        # Only parse if looks like JSON
        if response.text.strip().startswith(("{", "[")):
            return json.loads(response.text)
        else:
            print("❌ Not JSON:", response.text.strip()[:100])
            return None
    except Exception as e:
        print(f"Error fetching data: {e}")
        return None
...
def get_ercot_data():
    ...
    current_json = fetch_data(current_op)
    if not current_json:
        print(f"❌ No data for current_op: {current_op}")
        current_op = latest_friday - timedelta(weeks=1)
        prev_op = current_op - timedelta(weeks=5)
        current_json = fetch_data(current_op)
    ...
    print("✔️ Final status: current_df =", current_df is not None, "| previous_df =", previous_df is not None)
