...
def pivot_isone_json(json_data):
...
def fetch_isone_data(operating_date):
    ...
    try:
        ...
        print(f"Error fetching ISONE data: {e}")
        return None
...
def get_isone_data():
    ...
    # Initial fetch
    current_json = fetch_isone_data(current_op)
    previous_json = fetch_isone_data(previous_op)

    # Fallback: shift 1 month back if current is unavailable
    if not current_json:
    ...
    current_df = pivot_isone_json(current_json) if current_json else None
    previous_df = pivot_isone_json(previous_json) if previous_json else None

    # Prompt window: month after current_op
    ...
    print(current_op.strftime("%Y-%m-%d"),previous_op.strftime("%Y-%m-%d"))
